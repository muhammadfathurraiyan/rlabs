...
const Navbar = () => {
  ...
  return (
    <nav className="relative z-10 flex items-center justify-between py-5 px-24 max-sm:px-8">
      <Link
        href="/"
        className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-800 to-cyan-500"
      >
        RLabs.
      </Link>
      <div className="flex items-center gap-8 max-sm:gap-3">
        {links.map((link) => (
          <Link
            key={link.name}
            className={`relative hover:text-cyan-500 duration-300 ${
